      ...
      <section className="lg:hidden">
        <div className="space-y-5">
          <div className="overflow-hidden rounded-[2rem] border border-[var(--ff-accent-border)]/25 bg-gradient-to-br from-[var(--ff-accent-soft)]/25 via-[var(--ff-card)] to-[var(--ff-surface-2)] p-5 shadow-[0_18px_44px_rgba(0,0,0,0.24)]">
            ...
            <p className="mt-2 text-sm leading-relaxed text-[var(--ff-muted)]">
              Ajuste aparência, treino, backup e segurança com controles grandes para celular.
            </p>
          </div>
          ...
          {user?.provider === 'google' && !user?.hasPassword && (
            <div className="rounded-3xl border border-amber-500/25 bg-amber-500/10 p-4 text-amber-100">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="font-black">Crie uma senha de acesso</h2>
                  <p className="mt-1 text-sm leading-relaxed text-amber-100/75">
                    Assim você também poderá entrar com e-mail e senha.
                  </p>
                </div>
              </div>
            </div>
          ...
          <Card className="p-4">
            <SectionTitle
              icon={Palette}
              title="Aparência"
              description="Tema e cor principal do app."
            />
            ...
            <div className="mt-5 grid grid-cols-3 gap-2">
              <button
                ...
                className={
                  settings.themeMode === 'dark'
                    ? 'rounded-2xl border border-[var(--ff-accent-border)] bg-[var(--ff-accent-soft)] p-3 text-center text-[var(--ff-accent-text)]'
                    : 'rounded-2xl border border-[var(--ff-border)] bg-[var(--ff-surface-2)] p-3 text-center text-[var(--ff-muted)]'
                }
              >
                <Moon size={20} className="mx-auto" />
                <span className="mt-2 block text-xs font-black">Escuro</span>
              </button>
              ...
              >
                ...
                <span className="mt-2 block text-xs font-black">Claro</span>
              </button>
              ...
              >
                ...
                <span className="mt-2 block text-xs font-black">Sistema</span>
              </button>
            </div>
                  ...
                  <button
                    ...
                    className={
                      settings.accentColor === key
                        ? 'flex shrink-0 items-center gap-2 rounded-full border border-[var(--ff-accent-border)] bg-[var(--ff-accent-soft)] px-3 py-2 text-xs font-black text-[var(--ff-accent-text)]'
                        : 'flex shrink-0 items-center gap-2 rounded-full border border-[var(--ff-border)] bg-[var(--ff-surface-2)] px-3 py-2 text-xs font-bold text-[var(--ff-muted)]'
                    }
                  >
                    <span
            ...
            <div className="mt-5">
              <SettingToggleCard
                ...
                description="Reduz espaços em telas pequenas."
                active={settings.compactMobile}
                onChange={(value) => handleUpdateSetting('compactMobile', value)}
              />
            </div>
          </Card>
          ...
          <Card className="p-4">
            <SectionTitle
              ...
              title="Treino"
              description="Preferências rápidas para sessão e rotina."
            />
            ...
            <div className="mt-5 space-y-4">
              <Select
                ...
                onChange={(event) => handleUpdateSetting('defaultSetModel', event.target.value)}
              >
              ...
              <Select
                ...
                onChange={(event) => handleUpdateSetting('defaultRestTimer', event.target.value)}
              >
              ...
              <SettingToggleCard
                ...
                description="Evita finalizar um treino por acidente."
                active={settings.confirmBeforeFinishWorkout}
                onChange={(value) => handleUpdateSetting('confirmBeforeFinishWorkout', value)}
              />

              <SettingToggleCard
                title="Confirmar antes de cancelar"
                description="Evita perder a sessão atual."
                active={settings.confirmBeforeCancelWorkout}
                onChange={(value) => handleUpdateSetting('confirmBeforeCancelWorkout', value)}
              />
            </div>
          </Card>
          ...
          <Card className="p-4">
            ...
            <div className="mt-5 grid grid-cols-1 gap-3">
              <Button type="button" onClick={handleExportJson} disabled={Boolean(exportingType)}>
                <Download size={17} />
                {exportingType === 'json' ? 'Exportando...' : 'Exportar JSON'}
              </Button>

              <Button type="button" variant="secondary" onClick={handleExportCsv} disabled={Boolean(exportingType)}>
                <FileSpreadsheet size={17} />
                {exportingType === 'csv' ? 'Exportando...' : 'Exportar CSV'}
              </Button>

              <Button type="button" variant="secondary" onClick={handleExportPdf} disabled={Boolean(exportingType)}>
                <FileText size={17} />
                {exportingType === 'pdf' ? 'Exportando...' : 'Exportar PDF'}
              </Button>

              <label className="inline-flex h-11 cursor-pointer items-center justify-center gap-2 rounded-2xl border border-[var(--ff-border)] bg-[var(--ff-surface-2)] px-5 text-sm font-bold text-[var(--ff-text-soft)]">
                <FileJson size={17} />
                Importar JSON
                <input
                  type="file"
          ...
          <Card className="p-4">
            <SectionTitle
              icon={SlidersHorizontal}
              title={user?.hasPassword ? 'Alterar senha' : 'Criar senha'}
            ...
            <form onSubmit={handleSetPassword} className="mt-5 space-y-4">
              ...
              <Button type="submit" disabled={savingPassword} className="w-full">
                {savingPassword ? 'Salvando...' : user?.hasPassword ? 'Alterar senha' : 'Criar senha'}
              </Button>
            </form>
          </Card>
          ...
          <Card className="p-4">
            <SectionTitle
              icon={AlertTriangle}
              title="Área de risco"
              description="Restaurar preferências padrão."
            />
