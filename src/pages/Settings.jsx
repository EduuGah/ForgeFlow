    ...
    rows: [
      {
        ...
        label: "Perfil",
        href: "/profile",
        hint: "Foto, bio e dados do atleta",
      },
      {
        icon: Lock,
        label: "Conta",
      ...
      },
      {
        ...
        hint: "Lembretes e alertas do app",
      },
    ],
  },
  {
    ...
    rows: [
      {
        ...
        label: "Treinamentos",
        target: "training",
        hint: "Descanso, unidades e preferências",
      },
      {
        icon: Shield,
        label: "Privacidade e social",
        href: "/privacy",
        hint: "Dados, visibilidade e segurança",
      },
      {
        icon: ClipboardList,
        label: "Unidades",
        target: "training",
        hint: "Peso e padrões de treino",
      },
      ...
      },
      {
        ...
        label: "Tema",
        target: "appearance",
        hint: "Cor do app e aparência",
      },
      {
        icon: Download,
        label: "Exportar e importar dados",
        target: "backup",
        hint: "Backup e relatórios",
      },
    ],
  },
  {
    ...
    rows: [
      {
        icon: Info,
        label: "Guia de Arranque",
        target: "tutorial",
        hint: "Rever introdução do app",
      },
      {
        icon: ClipboardList,
        label: "Ajuda de Rotina",
        href: "/schedule",
        hint: "Agenda semanal de treinos",
      },
      {
        icon: HelpCircle,
  ...
  if (selectedPanel) {
    ...
    return (
      <section className="ff-settings-native-detail lg:hidden">
        <button
          type="button"
          className="ff-settings-native-back"
          onClick={onBack}
        >
          ‹ Voltar
        </button>
        <h2>{panelTitle}</h2>
        <p>
          Edite esta preferência abaixo. As alterações são salvas
          ...
          <div className="ff-settings-native-list">
            {group.rows.map((row) => {
              ...
              const content = (
                <>
                  {row.pro ? (
                    <span className="w-10 text-xs font-black text-[var(--ff-text)]">
                      PRO
                    </span>
                  ) : Icon ? (
                    <Icon size={25} className="w-10" />
                  ) : (
                    <span className="w-10" />
                  )}
              ...
              if (row.href) {
                return (
                  <a
                    key={row.label}
                    href={row.href}
                    className="ff-settings-native-row"
                  >
                    {content}
                  </a>
                );
...
function Settings() {
  ...
  useEffect(() => {
    ...
    const requestedPanel =
      location.state?.openSettingsPanel ||
      location.state?.activeSettingsSection ||
      params.get("section") ||
    ...
    window.setTimeout(() => {
      const target =
        document.querySelector(`[data-settings-panel="${requestedPanel}"]`) ||
        document.getElementById(`settings-${requestedPanel}`);
    ...
    async function loadSettings() {
      ...
      try {
        ...
        if (Array.isArray(remoteWorkouts)) {
          setWorkouts(
            remoteWorkouts.map((workout) => normalizeWorkoutFromApi(workout)),
          );
        } else {
  ...
  async function handleClearPwaCache() {
    try {
      ...
      showToast(
        "success",
        "Cache limpo",
        "Recarregue o app para buscar a versão mais recente.",
      );
    } catch (error) {
      console.error(error);
      showToast(
        ...
        "Não foi possível limpar o cache automaticamente.",
      );
    }
  }
  ...
  async function handleUpdateSetting(key, value) {
    ...
    const pendingKey =
      typeof key === "string" ? key : Object.keys(patch)[0] || "settings";
    ...
    try {
      ...
      showToast(
        "success",
        "Configuração salva",
        "A preferência foi salva na sua conta.",
      );
    } catch {
      setSyncStatus("local");
  ...
  function handleNativeSettingsNavigate(row) {
    if (!row?.target) {
      showToast(
        "info",
        row?.label || "Em breve",
        "Essa opção ainda será adicionada no ForgeFlow.",
      );
      return;
    }
  ...
  function panelClass(panel) {
    return selectedMobilePanel && selectedMobilePanel !== panel
      ? "hidden lg:block"
      : "";
  }
    ...
    <div className="ff-hevy-page ff-hevy-page-settings">
      ...
      <div className="hidden lg:block">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black tracking-[-0.055em]">
              Configurações
            </h1>
            <p className="mt-2 text-sm text-[var(--ff-muted)]">
              Ajuste visual, preferências úteis, backup e segurança da sua
              conta.
            </p>
          </div>
          <Badge variant={syncStatus === "idle" ? "purple" : "default"}>
      ...
      <section
        className={`ff-settings-layout grid grid-cols-1 gap-6 2xl:grid-cols-[minmax(0,1fr)_320px] ${
          selectedMobilePanel ? "is-mobile-panel-open" : "is-mobile-directory"
        }`}
      >
        <div className="ff-hevy-settings space-y-5 sm:space-y-6">
          <div
            id="settings-tutorial"
            data-settings-panel="tutorial"
            className={`scroll-mt-24 ${panelClass("tutorial")}`}
          >
            <SettingsTutorialSection />
          </div>
          ...
          <div
            ...
            data-settings-panel="maintenance"
            className={`scroll-mt-24 ${panelClass("maintenance")}`}
          >
          ...
          <div
            ...
            className={`scroll-mt-24 ${panelClass("appearance")}`}
          >
            <SettingsAppearanceSection
              settings={settings}
          ...
          <div
            ...
            data-settings-panel="training"
            className={`scroll-mt-24 ${panelClass("training")}`}
          >
            <SettingsTrainingPreferencesSection
              settings={settings}
              onUpdateSetting={handleUpdateSetting}
          ...
          <div
            ...
            className={`scroll-mt-24 ${panelClass("notifications")}`}
          >
          ...
          <div
            ...
            className={`scroll-mt-24 ${panelClass("backup")}`}
          >
            <SettingsBackupSection
          ...
          <div
            ...
            className={`scroll-mt-24 ${panelClass("security")}`}
          >
            <SettingsPasswordSection
              user={user}
        ...
        <div
          className={
            selectedMobilePanel && selectedMobilePanel !== "security"
              ? "hidden lg:block"
              : ""
          }
        >
