...
function getSessionVolume(session) {
  ...
  return (session.exercises || []).reduce((exerciseTotal, exercise) => {
    return exerciseTotal + (exercise.sets || []).reduce((setTotal, set) => {
      if (!set.completed && set.completed !== undefined) return setTotal
        ...
        <div className="space-y-5">
          <Card>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--ff-accent-text)]">
                  Mês selecionado
                </p>
                <h2 className="mt-1 text-2xl font-black capitalize text-[var(--ff-text)]">
                  {monthLabel}
                </h2>
              </div>
            ...
            <div className="mt-6 grid grid-cols-7 gap-2 text-center">
              {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((day) => (
                <div
                  key={day}
                  className="text-[11px] font-black uppercase tracking-wide text-[var(--ff-muted)]"
                >
                  {day}
                </div>
              ))}
              ...
              {days.map((day, index) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      'min-h-[70px] rounded-2xl border p-2 text-left transition sm:min-h-[88px]',
                      !day
                        ? 'border-transparent opacity-0'
                        : isSelected
                          ? 'border-[var(--ff-accent-border)] bg-[var(--ff-accent-soft)] shadow-[0_0_22px_var(--ff-accent-shadow)]/20'
                          : sessions.length > 0
                            ? 'border-[var(--ff-accent-border)]/40 bg-[var(--ff-card)] hover:bg-[var(--ff-card-hover)]'
                      ...
                      <>
                        <div className="flex items-center justify-between gap-2">
                          <span
                            className={
                              isToday
                                ? 'flex h-7 w-7 items-center justify-center rounded-full bg-[var(--ff-accent)] text-xs font-black text-white'
                                : 'text-sm font-black text-[var(--ff-text)]'
                            }
                          >
                            {day.getDate()}
                          </span>
                        ...
                        {sessions.length > 0 && (
                          <div className="mt-3 space-y-1">
                            <div className="h-1.5 rounded-full bg-[var(--ff-accent)]" />
                            <p className="line-clamp-1 text-[11px] font-bold text-[var(--ff-muted)]">
                              {sessions[0].workoutName || sessions[0].name || 'Treino'}
                            </p>
                          </div>
                        )}
                      </>
        ...
        <aside className="space-y-5">
          <Card>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-black text-[var(--ff-text)]">
                  Resumo do mês
                </h2>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-[var(--ff-border)] bg-[var(--ff-surface-2)] p-3">
                ...
                <p className="mt-2 text-xl font-black text-[var(--ff-text)]">{monthStats.totalWorkouts}</p>
                <p className="text-xs text-[var(--ff-muted)]">treinos</p>
              </div>

              <div className="rounded-2xl border border-[var(--ff-border)] bg-[var(--ff-surface-2)] p-3">
                <CalendarDays size={18} className="text-[var(--ff-accent-text)]" />
                <p className="mt-2 text-xl font-black text-[var(--ff-text)]">{monthStats.trainedDays}</p>
                <p className="text-xs text-[var(--ff-muted)]">dias</p>
              </div>

              <div className="rounded-2xl border border-[var(--ff-border)] bg-[var(--ff-surface-2)] p-3">
                <Flame size={18} className="text-orange-300" />
                <p className="mt-2 text-sm font-black text-[var(--ff-text)]">{formatVolume(monthStats.totalVolume)}</p>
                <p className="text-xs text-[var(--ff-muted)]">volume</p>
              </div>

              <div className="rounded-2xl border border-[var(--ff-border)] bg-[var(--ff-surface-2)] p-3">
                <Clock size={18} className="text-emerald-300" />
                <p className="mt-2 text-sm font-black text-[var(--ff-text)]">{formatDuration(monthStats.totalDuration)}</p>
                <p className="text-xs text-[var(--ff-muted)]">tempo</p>
              </div>
            </div>
          </Card>
          ...
          <Card>
            <h2 className="text-lg font-black text-[var(--ff-text)]">
              {selectedDateKey ? formatDate(selectedDateKey) : 'Selecione um dia'}
            </h2>

            <div className="mt-4 space-y-3">
              {selectedDateKey && selectedSessions.length === 0 && (
                <EmptyState
                  title="Sem treino neste dia"
                  description="Nenhum treino finalizado foi encontrado nessa data."
                />
              )}
              ...
              {!selectedDateKey && (
                <EmptyState
                  ...
                  description="Clique em um dia do calendário para ver os treinos."
                />
              )}

              {selectedSessions.map((session) => (
                <div
                  key={session.id || session._id || `${session.workoutName}-${getSessionDate(session)}`}
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-[var(--ff-muted)]">
                    {formatShortDate(getSessionDate(session))} • {formatDuration(session.durationSeconds || session.duration || 0)}
                  </p>

                  <p className="mt-2 text-xs font-bold text-[var(--ff-accent-text)]">
                    {formatVolume(getSessionVolume(session))}
                  </p>
                </div>
              ))}
            </div>
