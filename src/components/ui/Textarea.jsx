function Textarea({ label, rows = 4, className = '', error = '', ...props }) {
  return (
    <div className="min-w-0">
      ...
      <textarea
        rows={rows}
        className={`w-full resize-none rounded-2xl border border-[var(--ff-border)] bg-[var(--ff-input)] px-4 py-3 text-base font-medium text-[var(--ff-text)] outline-none transition placeholder:text-[var(--ff-muted-2)] hover:border-[var(--ff-border-strong)] focus:border-[var(--ff-accent-border)] focus:ring-2 focus:ring-[var(--ff-accent)]/10 disabled:cursor-not-allowed disabled:opacity-50 sm:text-sm ${className}`}
        {...props}
      />
